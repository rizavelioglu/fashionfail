...
class SimpleModel(pl.LightningModule):
    def __init__(
        ...
        *,
        ...
        self.model = construct_pretrained_model(
            self.n_classes, model_name="maskrcnn_resnet50_fpn_v2"
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        for loss_key, loss_value in loss_dict.items():
            self.log(
                f"{TRAIN_METRIC_PREFIX}{loss_key}", loss_value, batch_size=len(batch)
            )
        ...
        # Log summed loss.
        self.log(
            f"{TRAIN_METRIC_PREFIX}loss_sum", loss_sum.item(), batch_size=len(batch)
        )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for loss_key, loss_value in loss_dict.items():
            self.log(
                f"{VAL_METRIC_PREFIX}{loss_key}", loss_value, batch_size=len(batch)
            )
        ...
        # Log summed loss.
        self.log(f"{VAL_METRIC_PREFIX}loss_sum", loss_sum.item(), batch_size=len(batch))
